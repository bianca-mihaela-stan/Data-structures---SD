///problema discutata si la seminar
#include <iostream>
#include <fstream>
using namespace std;
ifstream f ("permutariab.in");
ofstream g ("permutariab.out");
int a[100005], b[100005], poza[100005],  pozb[100005], p[100005];
...
int main()
{
    ...
    for(int i=1; i<=n; i++)
    {
        f>>a[i];///citesc prima permutare
        poza[a[i]]=i;
    }
    for(int i=1; i<=n; i++)
    {
        f>>b[i];///citesc a doua permutare
        pozb[b[i]]=i;
        p[i]=poza[b[i]];///in p retin pozitia pe care se afla ementul curent din b in a
    }
    ///dupa merge o sa vreau ca pozitiile pe care se afla elementele din b in a sa fie 1 2 3 4 5 6...
    long long rez=0;
    merge_sort(p, 1, n, rez);
    g<<rez;
    return 0;
}
